...
W032 survived its first fresh holdout. Run `31937191372` matched aggregate compute within 0.061%: adaptive 6,088,678 nodes, uniform 6,092,208, shuffled 6,092,345. Mean deep-oracle regret was 2.89 / 3.57 / 4.64 cp. Adaptive beat uniform by 0.68 cp with bootstrap CI [0.22, 1.23] and 10/88/2 outcomes; it beat the shuffled-budget ablation by 1.75 cp, CI [0.57, 3.21], 12/87/1. This identified an allocation signal, not yet a stronger engine.

An adversarial A/A check then invalidated W032's external control representation. Two independent Python-driven adaptive runs made the same moves, assigned the same volatility flags, and retained the same root histories, yet consumed 76,012 versus 66,664 nodes on an eight-position calibration; the maximum per-position node gap was 69.9%. The cause is asynchronous host-stop latency after an iteration has already begun. W032 is therefore downgraded from a provisional mechanism win to a signal whose implementation is invalid. W034 may still test corpus transfer of that signal, but it cannot promote the external allocator.

Counterexample-guided refinement moved the one-time decision inside iterative deepening. Frozen candidate `63d7c506fc1d0bfafdd892ad762f43c1b68a1514` latches at the first completed root iteration beyond 15k nodes: five stable root moves stop immediately; a volatile root retains the 100k UCI cap. Its option-off build exactly matches P01 on the 3,210,480 / 1,286,415 / 2,451,202-node transcript suite. With the mechanism active, two independent processes matched every move and node count on the mini panel, 65,601/65,601 total. W035 now compares that deterministic arm with a uniform budget, a shuffled allocation, and Stockfish's native time manager in `nodestime=1` mode. Native clock size is secant-calibrated to aggregate nodes before oracle scoring, preserving Stockfish's existing stable-move, best-move-instability, falling-eval, and effort logic without wall-clock noise. The eight-position quality pilot was deliberately non-decisive; the frozen 100-position holdout decides whether the signal survives this stronger counterfactual.
