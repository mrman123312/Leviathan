...
def search(e,pos,nodes,multipv=1,searchmove=None):
    reset(e);e.send(f'setoption name MultiPV value {multipv}');e.send('isready');e.wait_for('readyok',20);e.drain();e.send(pos);e.send(('go '+(f'searchmoves {searchmove} ' if searchmove else '')+f'nodes {nodes}'));latest={};best=None;end=time.monotonic()+120
    while time.monotonic()<end:
        try:l=e.read(1)
        except queue.Empty:continue
        i=parse_info(l)
        if i and i.get('pv'):
            m=int(i.get('multipv',1));p=latest.get(m)
            if p is None or (i.get('depth',0),i.get('nodes',0))>=(p.get('depth',0),p.get('nodes',0)):latest[m]=i
