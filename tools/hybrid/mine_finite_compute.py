    ...
    try:
        with o.open('a',encoding='utf-8') as f:
            for pi,(pos,gid) in enumerate(positions):
                ...
                label_move=search(opp_label,pos,x.opponent_label_nodes,1).get('bestmove');bestcp=max(int(v.get('score_cp',0)) for v in lines.values())
