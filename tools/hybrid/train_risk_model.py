...
- optional untouched prospective dataset
- train-only normalization persisted in checkpoint
- masked class-balanced reply/risk losses
- third head predicts log1p centipawn regret
- CUDA / DirectML / CPU acceleration
- portable CPU-resident checkpoint tensors
"""
    ...
    for path in paths:
        ...
        with p.open(encoding='utf-8') as f:
            for lineno,line in enumerate(f,1):
                ...
                r=dict(r);r['_source_file']=str(p);r['_line']=lineno;r['_group']=group_key(r);out.append(r)
    return out


def group_key(r):
    if r.get('group_id'):raw='group:'+str(r['group_id'])
    elif r.get('position'):raw='position:'+str(r['position'])
    elif r.get('session_key'):raw='session:'+str(r['session_key'])
    elif r.get('generation') is not None:raw='generation:'+str(r.get('_source_file',''))+':'+str(r['generation'])
    else:raw='row:'+str(r.get('_source_file',''))+':'+str(r.get('_line',0))
...
def grouped_split(rows,holdout,seed):
    ...
    keys=list(groups);random.Random(seed).shuffle(keys);target=max(1,int(round(len(rows)*holdout)));valid_keys=set();count=0
    for k in keys:
        if count>=target and valid_keys:break
        valid_keys.add(k);count+=len(groups[k])
    train=[r for r in rows if r['_group'] not in valid_keys];valid=[r for r in rows if r['_group'] in valid_keys]
...
def pr_auc(scores,labels):
    ...
    order=sorted(range(len(scores)),key=lambda i:scores[i],reverse=True);tp=fp=0;last_recall=0.;area=0.
    for i in order:
        if labels[i]>.5:tp+=1
        else:fp+=1
        recall=tp/pos;precision=tp/max(1,tp+fp);area+=(recall-last_recall)*precision;last_recall=recall
    return area


def brier(scores,labels):return sum((s-y)**2 for s,y in zip(scores,labels))/len(scores) if scores else None
...
def top_fraction_capture(scores,values,frac=.2):
    ...
    n=max(1,int(math.ceil(len(scores)*frac)));idx=sorted(range(len(scores)),key=lambda i:scores[i],reverse=True)[:n]
...
def reply_top1(rows,probs):
    ...
    good=base=0
    for vals in by.values():good+=int(max(vals,key=lambda x:x[0])[1]>.5);base+=int(min(vals,key=lambda x:x[2])[1]>.5)
    return good/len(by),base/len(by)


def class_weights(rows,key):
    ys=[float(r[key]) for r in rows if r.get(key) is not None];pos=sum(y>.5 for y in ys);neg=len(ys)-pos
    return max(1.,neg/max(1,pos)) if ys else 1.


def metrics_for(rows,reply_probs,risks,regrets):
    ri=[i for i,r in enumerate(rows) if r.get('reply_label') is not None];ki=[i for i,r in enumerate(rows) if r.get('risk_label') is not None]
    rl=[float(rows[i]['reply_label']) for i in ri];rp=[reply_probs[i] for i in ri];kl=[float(rows[i]['risk_label']) for i in ki];kp=[risks[i] for i in ki];rv=[float(rows[i].get('regret_cp') or 0.) for i in ki]
    top1,base=reply_top1([rows[i] for i in ri],rp);mae=None;reg_idx=[i for i,r in enumerate(rows) if r.get('regret_cp') is not None]
    if reg_idx:mae=sum(abs(regrets[i]-float(rows[i].get('regret_cp') or 0.)) for i in reg_idx)/len(reg_idx)
    return {'rows':len(rows),'groups':len({r['_group'] for r in rows}),'reply_positive_rate':sum(rl)/len(rl) if rl else None,'reply_auc':roc_auc(rp,rl),'reply_pr_auc':pr_auc(rp,rl),'reply_brier':brier(rp,rl),'reply_top1_accuracy':top1,'reply_rank1_baseline':base,'risk_positive_rate':sum(kl)/len(kl) if kl else None,'risk_auc':roc_auc(kp,kl),'risk_pr_auc':pr_auc(kp,kl),'risk_brier':brier(kp,kl),'risk_top20_regret_capture':top_fraction_capture(kp,rv,.2) if kp else None,'risk_top10_regret_capture':top_fraction_capture(kp,rv,.1) if kp else None,'regret_mae_cp':mae}
...
def main():
    ...
    opt=torch.optim.AdamW(model.parameters(),lr=x.lr,weight_decay=1e-4);bce=torch.nn.BCEWithLogitsLoss(reduction='none');smooth=torch.nn.SmoothL1Loss(reduction='none');wr=class_weights(train,'reply_label');wk=class_weights(train,'risk_label')
    def td(values,dtype=torch.float32):return torch.tensor(values,dtype=dtype).to(device)
    def tensors(batch):
        X=td([r['features'] for r in batch]);X=(X-mean_d)/std_d;yr=td([float(r.get('reply_label') or 0) for r in batch]);mr=td([1. if r.get('reply_label') is not None else 0. for r in batch]);yk=td([float(r.get('risk_label') or 0) for r in batch]);mk=td([1. if r.get('risk_label') is not None else 0. for r in batch]);yg=td([math.log1p(max(0.,float(r.get('regret_cp') or 0.))) for r in batch]);mg=td([1. if r.get('regret_cp') is not None else 0. for r in batch]);return X,yr,mr,yk,mk,yg,mg
    def infer(batch):
        model.eval();X,_,_,_,_,_,_=tensors(batch)
        with torch.inference_mode():z=model(X);rp=torch.sigmoid(z[:,0]).detach().cpu().tolist();rk=torch.sigmoid(z[:,1]).detach().cpu().tolist();rg=torch.expm1(torch.clamp(z[:,2],min=0,max=math.log1p(1000))).detach().cpu().tolist()
        return rp,rk,rg
    best=None;best_state=None;stale=0;epochs_ran=0
    for epoch in range(x.epochs):
        epochs_ran=epoch+1;random.shuffle(train);model.train()
        for i in range(0,len(train),x.batch_size):
            batch=train[i:i+x.batch_size];X,yr,mr,yk,mk,yg,mg=tensors(batch);z=model(X);rw=torch.where(yr>.5,torch.full_like(yr,wr),torch.ones_like(yr));kw=torch.where(yk>.5,torch.full_like(yk,wk),torch.ones_like(yk));lr=(bce(z[:,0],yr)*rw*mr).sum()/mr.sum().clamp_min(1);lk=(bce(z[:,1],yk)*kw*mk).sum()/mk.sum().clamp_min(1);lg=(smooth(z[:,2],yg)*mg).sum()/mg.sum().clamp_min(1);loss=lr+lk+0.35*lg;opt.zero_grad(set_to_none=True);loss.backward()
            ...
            opt.step()
        rp,rk,rg=infer(valid);vm=metrics_for(valid,rp,rk,rg);score=(vm.get('risk_auc') or .5)+(vm.get('risk_top20_regret_capture') or .2)+0.25*(vm.get('reply_top1_accuracy') or 0.)
    ...
    gate_metrics=pros_metrics or valid_metrics;checks,promote=passes(gate_metrics,x.min_risk_auc,x.min_top20_regret_capture,x.min_reply_top1_gain)
    metrics={'version':'p18.3','device':device_name,'epochs_ran':epochs_ran,'train_rows':len(train),'valid':valid_metrics,'prospective':pros_metrics,'promotion_checks':checks,'promote':promote,'seed':x.seed,'reply_pos_weight':wr,'risk_pos_weight':wk}
