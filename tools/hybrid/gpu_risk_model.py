...
class GpuRiskScorer:
    ...
    def _score_model(self,rows):
        ...
        with torch.inference_mode():
            z=self.model(x);reply_prob=torch.softmax(z[:,0],dim=0);risk=torch.sigmoid(z[:,1]);regret=torch.expm1(torch.nn.functional.softplus(z[:,2])) if self.heads>=3 else 25*risk
