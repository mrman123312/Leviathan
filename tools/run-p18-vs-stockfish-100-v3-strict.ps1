...
$RepoCommit='0c12bd045e1714ac265e353d2c1b6ed9708b5f6a'
...
Write-Host '=== P18.5 STRICT MATCH-FIRST / RACE-SAFE PONDER ===' -ForegroundColor Magenta
Write-Host 'No dynamic annealing. No silent game retries. Any protocol fault aborts visibly.' -ForegroundColor Yellow
...
Write-Host '=== SYNC RACE-SAFE SOURCE / PRESERVE LOCAL RESULTS ===' -ForegroundColor Cyan
if(Test-Path (Join-Path $Work '.git')){
  git -C $Work fetch --depth 1 origin $RepoCommit | Out-Host; Assert-LastExit 'git fetch P18.5'
  git -C $Work reset --hard FETCH_HEAD | Out-Host; Assert-LastExit 'git reset P18.5'
}else{
  git clone --no-checkout https://github.com/mrman123312/Leviathan.git $Work | Out-Host; Assert-LastExit 'git clone'
  git -C $Work fetch --depth 1 origin $RepoCommit | Out-Host; Assert-LastExit 'git fetch P18.5'
  git -C $Work checkout --detach FETCH_HEAD | Out-Host; Assert-LastExit 'git checkout P18.5'
}
...
else{throw 'No P18 advisor checkpoint found. Existing partial-training checkpoint was expected.'}
...
Write-Host '=== START FRESH STRICT 100-GAME MATCH ===' -ForegroundColor Magenta
Write-Host 'WIN/LOSS => immediate same-opening/same-color learned-advisor-OFF replay. DRAW => next game.' -ForegroundColor Yellow
Write-Host 'A timeout/crash is NOT retried or counted; the launcher aborts and can be rerun to resume clean completed games.' -ForegroundColor Yellow

$Harness=Join-Path $Work 'tools\hybrid\run_p18_vs_stockfish_100_strict.py'
$HybridScript=Join-Path $Work 'tools\hybrid\leviathan_hybrid_uci_v3.py'
$MatchOut=Join-Path $Work 'local_results\hybrid\p18-vs-stockfish-100-match-first-v3-strict'
...
Assert-LastExit 'P18.5 strict 100-game match + decisive no-GPU ablations'

Write-Host '=== STRICT 100-GAME MATCH COMPLETE ===' -ForegroundColor Green
