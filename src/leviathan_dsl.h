...
// Hot-path variant: caller has already established ready(). Keeping this
// separate avoids reconstructing readiness and state several times per LMR
// candidate while preserving exactly the same interpreter semantics.
...
inline int lmr_adjustment(Depth depth,
                          ...
                          Value staticEval,
                          Value alpha) {
    // v3 lossless hot-path rule: an inactive DSL must cost only the readiness
    // branch. Previously all twelve features were materialized before eval()
    // discovered that the DSL was disabled.
    if (!ready())
        return 0;
