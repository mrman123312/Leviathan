...
class ModelRegistry:
    """Read-only registry with explicit giant-checkpoint acquisition permission."""
    ...
    def from_toml(cls, path: Path = DEFAULT_REGISTRY_PATH) -> "ModelRegistry":
        with path.open("rb") as f:
            raw = tomllib.load(f)
        return cls([ModelSpec(**item) for item in raw.get("models", [])])
    ...
    def by_role(self, role_fragment: str) -> tuple[ModelSpec, ...]:
        return tuple(model for model in self.all() if role_fragment.lower() in model.role.lower())
    ...
    def canonical_substrate(self) -> ModelSpec:
        ...
        if len(canonical) != 1:
            raise ValueError(f"Leviathan requires one canonical semantic substrate; found {len(canonical)}")
        return canonical[0]
    def require_download_permission(self, model_id: str, *, allow_disabled: bool = False) -> ModelSpec:
        ...
        if not model.enabled_for_download and not allow_disabled:
            raise PermissionError(f"{model_id} disabled for automatic download; explicit opt-in required")
        return model
