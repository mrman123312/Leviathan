...
Checkpoint-native DeepSeek weights expose these as separate `w1` gate, `w3` up and `w2` down projections. Serving engines may pack `w1` and `w3` into a fused gate/up matrix; that packed representation is a storage/kernel view of the same logical channel tile, not a different MoP definition.
...
Require the canonical 61-layer config and all 64 weight shards. The safetensors index must also reference the complete canonical shard set.
...
Validate a pinned downloaded config without claiming the weights are present:
...
The manifest is the handoff contract for the tensor/kernel implementation. It records the canonical model identity, source fingerprint, full-checkpoint status, immutable revision and MoP tile geometry.
...
- strict detailed V4 config validation against the machine-readable spec,
- full 64-shard presence and safetensors-index validation,
- MoP tile geometry,
- exact expert-route -> tile-route expansion,
- checkpoint-native and fused-serving tile-coordinate views,
- immutable-revision transplant manifest,
...
- rewriting/repacking the 1.6 TB V4 weights into a production tile-aware kernel storage format,
